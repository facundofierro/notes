...
1.  **Session Cleanup**
    - Ensure that starting a _new_ run for a tool overwrites the old session cleanly (or archives it if we wanted history, but for now overwriting is fine per task implication).
