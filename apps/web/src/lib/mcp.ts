  ...
  create: {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        state: {
          type: "string",
          enum: ["backlog", "priority", "fixes", "pending", "doing", "done"],
          default: "pending",
    ...
    try {
      ...
      switch (name) {
        case "create": {
          ...
          let targetDir =
            type === "task"
              ? path.join(agelumPath, "work/tasks", state)
        ...
        case "move": {
          ...
          let sourceDir = path.join(agelumPath, "work/tasks", fromState);
          if (epic) {
          ...
          let targetDir = path.join(agelumPath, "work/tasks", toState);
          if (epic) {
            targetDir = path.join(targetDir, sanitizeFileNamePart(epic));
          }
        ...
        case "get": {
          ...
          if (!resolvedFileName) {
            ...
            if (type === "task") {
              if (priority !== undefined && storyPoints !== undefined) {
                resolvedFileName = buildFileName({
                  type,
                  title,
                  priority,
                  storyPoints,
          ...
          if (type === "task") {
            if (state) {
              searchPath = path.join(agelumPath, "work/tasks", state);
              if (epic) {
              ...
              for (const s of states) {
                let dir = path.join(agelumPath, "work/tasks", s);
