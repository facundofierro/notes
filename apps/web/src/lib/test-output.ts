...
const EXIT_CODE_REGEX =
  /Test\s+.+?\s+exited with code\s+(-?\d+)/g;
...
const STDERR_ERRORS_DETECTED_REGEX =
  /errors detected in (?:stdout|stderr)/i;
...
export function inferTestExecutionStatus(
  ...
  const hasNonZeroExit =
    exitCodes.some((c) => c !== 0);
  const hasZeroExit =
    exitCodes.some((c) => c === 0);

  const hasRuntimeErrorLike =
    ERROR_LIKE_REGEX.test(output) ||
    STDERR_ERRORS_DETECTED_REGEX.test(
      output,
    );

  if (hasNonZeroExit) return "failure";
  if (hasZeroExit && hasRuntimeErrorLike)
    return "failure";
