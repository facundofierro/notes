...
export function AgentExecutionStatus({
  ...
  const hasOutput = (output && output.trim().length > 0) || (error && error.trim().length > 0)
    ...
    <div className={cn('space-y-2', className)}>
      ...
      {isExpanded && hasOutput && (
        <div className="rounded-md border bg-muted/50 p-3">
          {error && (
            <div className="mb-2">
              <div className="mb-1 text-sm font-semibold text-destructive">Error:</div>
          ...
          {output && (
            <div>
              {error && <div className="mb-1 text-sm font-semibold">Output:</div>}
