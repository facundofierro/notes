...
export function BranchSwitcher({
  ...
  const fetchBranches = React.useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/git?action=branches&path=${encodeURIComponent(repoPath)}`,
      );
      if (res.ok) {
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent
        className="w-[300px] p-0 border-border bg-popover/95 backdrop-blur-md shadow-2xl overflow-hidden"
        align="start"
      >
          ...
          <ScrollArea className="h-[240px] p-1.5">
            ...
            <div className="space-y-0.5">
              {filteredBranches.map((branch) => (
                ...
                >
                  <GitBranch
                    className={cn(
                      "mr-2.5 h-3.5 w-3.5 transition-colors",
                      branch === currentBranch
                        ? "text-primary"
            ...
            {search && !exactMatch && (
              ...
              >
                ...
                <span>
                  Create branch:{" "}
                  <span className="underline decoration-primary/30 underline-offset-2 ml-1">
                    &quot;{search}&quot;
                  </span>
                </span>
              </div>
