...
export function GitHubPRsPanel({
  ...
  const [localCheckoutLoading, setLocalCheckoutLoading] = React.useState<
    number | null
  >(null);
  const [createOpen, setCreateOpen] = React.useState(false);
  const [internalActionLoading, setInternalActionLoading] = React.useState<
    string | null
  >(null);
  const actionLoading =
    externalActionLoading !== undefined
      ? externalActionLoading
      : internalActionLoading;

  // Internal state if not controlled
  const [internalSelectedPRNumber, setInternalSelectedPRNumber] =
    React.useState<number | null>(null);
  const selectedPRNumber =
    externalSelectedPRNumber !== undefined
      ? externalSelectedPRNumber
      : internalSelectedPRNumber;
  ...
  const fetchPRs = React.useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/github?action=list&path=${encodeURIComponent(repoPath)}`,
      );
      if (!res.ok) {
  ...
  const fetchPRDetails = React.useCallback(
    async (number: number) => {
      ...
      try {
        const res = await fetch(
          `/api/github?action=details&path=${encodeURIComponent(repoPath)}&pr=${number}`,
        );
  ...
  const currentCheckoutLoading =
    isCheckingOut !== undefined ? isCheckingOut : localCheckoutLoading;
  ...
  const getReviewBadge = (decision?: string) => {
    if (decision === "APPROVED")
      return (
        <Badge
          variant="default"
          className="bg-green-500/10 text-green-600 hover:bg-green-500/20 border-green-500/20 text-[10px] h-5 px-1.5"
        >
          Approved
        </Badge>
      );
      ...
      <div className="flex items-center justify-between p-3 border-b border-border bg-background/50 backdrop-blur-sm sticky top-0 z-10">
        ...
        <div className="flex items-center gap-1">
          ...
          <Button
            ...
            size="icon"
            onClick={() =>
              selectedPRNumber ? fetchPRDetails(selectedPRNumber) : fetchPRs()
            }
          ...
          >
            <RefreshCw
              className={cn(
                "w-3.5 h-3.5",
                (loading || detailsLoading) && "animate-spin",
              )}
            />
          </Button>
        </div>
            ...
            {prs.map((pr) => (
              ...
              >
                <div className="flex justify-between items-start gap-2">
                  ...
                  <Badge
                    variant="outline"
                    className="text-[10px] h-5 px-1.5 flex-shrink-0 bg-background"
                  >
                    #{pr.number}
                  </Badge>
                </div>
