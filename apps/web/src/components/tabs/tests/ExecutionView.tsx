...
import { X, Image as ImageIcon, Terminal, Loader2, Clock, CheckCircle2, XCircle } from "lucide-react";
  ...
  const displayLogs = React.useMemo(() => {
    return logs.filter(line => {
      try {
        ...
        if (parsed.type === "exec_start" || parsed.type === "exec_complete") return false;
      } catch { /* not JSON, keep */ }
      return true;
    });
  }, [logs]);
    ...
    <div className="flex flex-col h-full bg-zinc-950/50">
      ...
      <div className="flex items-center justify-between px-5 py-3 border-b border-white/[0.04] bg-zinc-950/80 backdrop-blur-sm">
        <div className="flex items-center gap-3">
          <StatusIcon className={cn("w-4 h-4", config.color, status === "running" && "animate-spin")} />
          <div>
            <h3 className="text-sm font-medium text-zinc-200">Execution</h3>
            <span className="text-[10px] text-zinc-600 font-mono">{executionId}</span>
          </div>
        </div>
        <Button
          variant="ghost"
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-[45%] flex flex-col border-r border-white/[0.04]">
          <div className="px-4 py-2.5 border-b border-white/[0.04] flex items-center gap-2">
            ...
            <span className="text-[11px] font-medium text-zinc-400">Screenshots</span>
            <span className="text-[10px] text-zinc-600 ml-auto">{screenshots.length}</span>
          </div>
          ...
          <ScrollArea className="flex-1">
            {screenshots.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 text-zinc-600">
                ...
                <span className="text-xs">{isRunning ? "Waiting for screenshots..." : "No screenshots"}</span>
              </div>
            ) : (
            ...
            <div className="p-4 font-mono text-[11px] leading-relaxed text-green-400/80 whitespace-pre-wrap break-all min-h-full bg-black/30">
              {displayLogs.length > 0 ? (
                displayLogs.map((line, i) => (
                  <div key={i} className="hover:bg-white/[0.02] px-1 -mx-1 rounded">
                    {line}
                  </div>
                ))
              ) : (
                <span className="text-zinc-600">{isRunning ? "Waiting for output..." : "No logs available."}</span>
              )}
              {isRunning && (
