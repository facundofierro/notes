...
export function TestEditor({ testId, onBack }: TestEditorProps) {
  ...
  const [activeTab, setActiveTab] = React.useState<"steps" | "results">(
    "steps",
  );
                  ...
                  return (
                    ...
                    >
                      <div className="p-3 flex flex-col items-center gap-2 text-center h-full justify-center">
                        <div
                          className={`p-2 rounded-lg transition-colors duration-300 ${isSelected ? "bg-emerald-500/10 text-emerald-400" : "bg-white/5 text-zinc-500 group-hover:text-zinc-200"}`}
                        >
                          <Icon className="w-5 h-5" />
                        </div>
                        <div>
                          <div
                            className={`text-xs font-bold mb-0.5 ${isSelected ? "text-zinc-100" : "text-zinc-400 group-hover:text-zinc-200"}`}
                          >
                            {t.label}
                          </div>
