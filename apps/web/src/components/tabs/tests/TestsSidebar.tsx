...
export function TestsSidebar({
  ...
  const filteredGroups = React.useMemo(() => {
    ...
    Object.keys(grouped).forEach((g) => {
      const matches = grouped[g].filter((t) =>
                ...
                >
                  ...
                  <Folder
                    className={cn(
                      ...
                      openGroups[group]
                        ? "text-amber-400"
                        : "text-zinc-600 group-hover/folder:text-zinc-400",
                    )}
                  />
                  <span className="text-xs font-semibold uppercase tracking-wider">
                    {group}
                  </span>
                  <span className="ml-auto text-[10px] text-zinc-600 bg-white/[0.02] px-1.5 rounded">
                    {groupTests.length}
                  </span>
                </button>
