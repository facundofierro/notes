...
function getStepLabel(step: TestStep): string {
  switch (step.action) {
    case "open":
      return step.url || "URL";
    case "click":
      return step.selector || "element";
    case "type":
      return step.text ? `"${step.text}"` : step.selector || "input";
    case "wait":
      return step.value || "...";
    case "prompt":
      return step.instruction || "AI action";
    case "screenshot":
    ...
    case "verifyVisible":
      return step.selector || "element";
    case "setViewport":
      return `${step.width || 1280}×${step.height || 720}`;
    default:
      return step.action;
  }
}
...
export function TestStepVisualizer({
  steps,
  className,
  compact,
}: TestStepVisualizerProps) {
  if (steps.length === 0) {
    return (
      <div
        className={cn(
          "flex items-center justify-center py-6 text-zinc-600 text-xs",
          className,
        )}
      >
        No steps defined
