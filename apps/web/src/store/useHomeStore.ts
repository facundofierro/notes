    ...
    (set, get) => ({
      ...
      fetchRepositories: async () => {
        try {
          ...
          set((state) => {
            const updates: Partial<HomeState> = { repositories: nextRepos };
            ...
            if (nextRepos.length > 0) {
              ...
              if (!currentSelected || !nextRepos.some((r) => r.name === currentSelected)) {
                updates.selectedRepo = nextRepos[0].name;
                
                // Also ensure projectState exists for the new selection
                const projectStates = { ...state.projectStates };
                if (!projectStates[nextRepos[0].name]) {
                  projectStates[nextRepos[0].name] = createDefaultProjectState();
                }
    ...
    }),
    {
      ...
      partialize: (state) => ({
        ...
        projectStates: Object.fromEntries(
          Object.entries(state.projectStates).map(([repo, pState]) => [
            repo,
            {
              viewMode: pState.viewMode,
              testViewMode: pState.testViewMode,
              activeTerminalId: pState.activeTerminalId,
            },
          ])
        ),
      }),
