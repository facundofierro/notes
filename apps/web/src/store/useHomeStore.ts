...
export interface HomeState {
  ...
  setProjectState: (
    updater: (prev: ProjectState) => Partial<ProjectState>,
  ) => void;
  ...
  registerTerminalSession: (session: TerminalSessionInfo) => void;
  updateTerminalSession: (
    processId: string,
    updates: Partial<TerminalSessionInfo>,
  ) => void;
  removeTerminalSession: (processId: string) => void;
  ...
  persist(
    (set, get) => ({
      ...
      getProjectState: () => {
        ...
        return (
          (selectedRepo ? projectStates[selectedRepo] : null) ||
      ...
      setAppLogs: (updater) => {
        get().setProjectState((prev) => ({
          appLogs:
            typeof updater === "function" ? updater(prev.appLogs) : updater,
          ...
          terminals: prev.terminals?.map((t) =>
            t.id === id
              ? {
                  ...t,
                  output:
                    typeof updater === "function" ? updater(t.output) : updater,
                }
              : t,
          ),
          lastTerminalActivity: Date.now(),
      ...
      getTerminalSessionForContext: (contextKey) => {
        ...
        return state.terminalSessions.find((s) => s.contextKey === contextKey);
      },
      ...
      resetSettings: async () => {
        try {
          ...
          set({
            settingsError: "Failed to reset settings",
          });
          throw err;
        } finally {
          set({
      ...
      fetchRepositories: async () => {
        try {
          ...
          set((state) => {
            ...
            if (data.basePath) updates.basePath = data.basePath;

            // If we don't have a selected repo or it's not in the new list, pick the first one
            if (nextRepos.length > 0) {
              const currentSelected = state.selectedRepo;
              if (
                !currentSelected ||
                !nextRepos.some((r) => r.name === currentSelected)
              ) {
                updates.selectedRepo = nextRepos[0].name;

                // Also ensure projectState exists for the new selection
                const projectStates = {
                  ...state.projectStates,
                };
                projectStates[nextRepos[0].name] = {
                  ...createDefaultProjectState(),
                  ...(projectStates[nextRepos[0].name] || {}),
                };
                updates.projectStates = projectStates;
              }
            }
      ...
      handleStopApp: async () => {
        ...
        const project = settings.projects?.find((p) => p.name === selectedRepo);
        const stopCommand =
          project?.commands?.stop || pState.projectConfig?.commands?.stop;
        ...
        try {
          ...
          console.error(
            "Failed to send stop signals/command to terminal:",
            error,
          );
        }
      },
      ...
      handleRunTest: async (path: string) => {
        ...
        try {
          ...
          while (true) {
            ...
            fullOutput += text;
            get().setProjectState((prev) => ({
              testOutput: prev.testOutput + text,
            }));
          }
        } catch (error) {
          fullOutput += "\nError running test";
          get().setProjectState((prev) => ({
            testOutput: prev.testOutput + "\nError running test",
          }));
        } finally {
          get().setProjectState(() => ({
            isTestRunning: false,
      ...
      saveFile: async (opts) => {
        ...
        get().setProjectState((prev) => {
          ...
          return {
            selectedFile:
              prev.selectedFile && prev.selectedFile.path === opts.path
    ...
    }),
    {
      ...
      onRehydrateStorage: () => (state) => {
        ...
        if (
          state &&
          state.selectedRepo &&
          !state.projectStates[state.selectedRepo]
        ) {
