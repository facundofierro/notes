...
import { ensureEnvFileMissingOnly, readDotenvFile } from "@/lib/env-file";

function findAgelumTestsDir(inputPath: string): {
  projectDir: string;
  testsDir: string;
} | null {
  ...
  while (current !== "/") {
    ...
    if (base === "src" && parent === "tests" && grandParent === "work") {
      const greatGrandParent = path.basename(
        path.dirname(path.dirname(path.dirname(current))),
      );
      if (greatGrandParent === ".agelum") {
        ...
        if (!fs.existsSync(newTestsDir) && fs.existsSync(current)) {
          try {
    ...
    if (base === "tests" && parent === "work" && grandParent === ".agelum") {
      const legacySrc = path.join(current, "src");
      const repoRoot = path.dirname(path.dirname(path.dirname(current)));
      const projectDir = path.join(repoRoot, "agelum-test");
      const newTestsDir = path.join(projectDir, "tests");
      if (!fs.existsSync(newTestsDir) && fs.existsSync(legacySrc)) {
        try {
          fs.mkdirSync(projectDir, {
            recursive: true,
          });
...
function ensureStagehandTestProject(projectDir: string) {
...
function hasStagehandInstalled(projectDir: string): boolean {
...
export async function POST(request: Request) {
  try {
    ...
    if (fs.statSync(testPath).isFile()) {
      projectRoot = path.dirname(testPath);
    }
    ...
    if (projectRoot === "/") {
      ...
      projectRoot = path.dirname(path.dirname(testPath));
    }
    ...
    const chunkLooksErrorLike = (text: string) => {
    ...
    const stream = new ReadableStream({
      async start(controller) {
        const agelumTestsDir = findAgelumTestsDir(testPath);

        const agelumTestProjectDir = agelumTestsDir?.projectDir ?? null;
        const runCwd = agelumTestProjectDir ?? projectRoot;

        if (agelumTestProjectDir) {
          ensureStagehandTestProject(agelumTestProjectDir);
        }

        const dotenvValues = agelumTestProjectDir
          ? readDotenvFile(path.join(agelumTestProjectDir, ".env"))
          : {};
        ...
        if (agelumTestProjectDir) {
          ensureAgelumTestEnvFiles(agelumTestProjectDir, seedEntries);
        }
        ...
        const baseEnv: NodeJS.ProcessEnv = {
          ...
          BROWSERBASE_API_KEY:
            dotenvValues.BROWSERBASE_API_KEY || seedEntries.BROWSERBASE_API_KEY,
          OPENAI_API_KEY:
            dotenvValues.OPENAI_API_KEY || seedEntries.OPENAI_API_KEY,
          ANTHROPIC_API_KEY:
            dotenvValues.ANTHROPIC_API_KEY || seedEntries.ANTHROPIC_API_KEY,
          GOOGLE_GENERATIVE_AI_API_KEY:
            dotenvValues.GOOGLE_GENERATIVE_AI_API_KEY ||
            seedEntries.GOOGLE_GENERATIVE_AI_API_KEY,
          XAI_API_KEY: dotenvValues.XAI_API_KEY || seedEntries.XAI_API_KEY,
        };
        ...
        if (
          ...
          !hasStagehandInstalled(agelumTestProjectDir)
        ) {
          controller.enqueue(
            encoder.encode(`\n\n--- Installing test dependencies ---\n`),
          );
          const installCode = await runCommandStreaming(controller, encoder, {
            command: "pnpm",
            args: ["install"],
        ...
        const tsxPath = tsxCandidatePaths.find((p) => fs.existsSync(p)) ?? null;
        ...
        for (const file of filesToRun) {
          ...
          const command = tsxPath ?? "npx";
          const args = tsxPath ? [file] : ["tsx", file];
          const code = await runCommandStreaming(controller, encoder, {
            command,
            args,
            cwd: runCwd,
            env: baseEnv,
            onStdoutChunk: (chunk) => {
              ...
              if (!sawErrorLikeInStdout && chunkLooksErrorLike(stdoutTail)) {
                sawErrorLikeInStdout = true;
              }
            },
            onStderrChunk: (chunk) => {
              stderrTail = (stderrTail + chunk).slice(-4000);
              if (!sawErrorLikeInStderr && chunkLooksErrorLike(stderrTail)) {
          ...
          const errorDetectedNote =
            sawErrorLikeInStdout || sawErrorLikeInStderr
              ? ` (errors detected in ${[
                  sawErrorLikeInStdout ? "stdout" : null,
                  sawErrorLikeInStderr ? "stderr" : null,
                ]
                  .filter(Boolean)
                  .join("+")})`
              : "";
