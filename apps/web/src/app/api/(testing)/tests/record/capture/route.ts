...
function runAgentBrowser(args: string[]): Promise<{ code: number | null; stdout: string; stderr: string }> {
  return new Promise(async (resolve) => {
    ...
    child.on("close", (code) => {
      console.log(`[RecordCapture] Command "${args[0]}" closed with code ${code}`);
    ...
    setTimeout(() => {
      if (child.exitCode === null) {
        console.warn(`[RecordCapture] Command "${args[0]}" timed out, killing...`);
        child.kill();
...
export async function POST() {
  try {
    ...
    if (screenshotResult.code === 0 && fs.existsSync(tmpPath)) {
      ...
      console.log(`[RecordCapture] Screenshot captured, size: ${stats.size} bytes`);
      screenshot = fs.readFileSync(tmpPath).toString("base64");
      try { fs.unlinkSync(tmpPath); } catch { /* ignore cleanup errors */ }
    } else {
      console.warn("[RecordCapture] Screenshot capture failed or file not found", {
    ...
    if (snapshotResult.code !== 0) {
      ...
      console.log(`[RecordCapture] Snapshot captured, length: ${snapshot.length} chars`);
    }
