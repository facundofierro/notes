...
export async function GET(request: Request) {
  ...
  const deferPrompt =
    searchParams.get("deferPrompt") === "1" ||
    searchParams.get("deferPrompt") === "true";
  const createSession =
    searchParams.get("createSession") === "1" ||
    searchParams.get("createSession") === "true";
  ...
  try {
    ...
    if (prompt || deferPrompt || createSession) {
      try {
    ...
    return NextResponse.json(
      {
        error: (error as Error)?.message || "Failed to ensure OpenCode server",
      },
      { status: 500 },
    );
  }
